...
static void uw_id_tdp(void)
{
	if (uw_feats->model == UW_MODEL_PH4TUX) {
	...
	} else if (dmi_match(DMI_BOARD_NAME, "X5SP4NAG") || 
               dmi_match(DMI_PRODUCT_SKU, "3297274")) {
        ...
        if (uw_feats) {
            uw_feats->uniwill_has_universal_ec_fan_control = true;
            uw_feats->uniwill_custom_profile_mode_needed = true; 
        }
